...
import { InvokeLLM } from "../../integrations/Core"; // 👈 bring in AI

// … your activities array stays exactly the same …
const activities = [ 
  { id: "journal",
   title: "Write in Journal",
   description: "Express your thoughts and feelings in a safe space", icon: BookOpen,
  ...
  { 
    ...
    icon: Users, color: "from-green-500 to-emerald-500",
    page: "Community", 
  ...
  { 
    ...
    { 
      ...
      description: "Get relationship advice and self-improvement tips", icon: Heart, 
      color: "from-pink-500 to-rose-500", 
      page: "Growth", 
      benefits: "Builds confidence and emotional intelligence", moodMatch: ["confused", "sad", "lonely", "happy"]
    }]
export default function ActivityGrid({ currentMood, userPreferences = [] }) {
  const navigate = useNavigate();
  const [aiTips, setAiTips] = useState({}); // {activityId: message}
  ...
  const sortedActivities = [...activities].sort((a, b) => {
    ...
    return (
      bMatchesMood + bInPreferences - (aMatchesMood + aInPreferences)
    );
  });

  // handle AI call for one activity
  const getAiTip = async (activity) => {
    // prevent re-calling if we already have one
    if (aiTips[activity.id]) return;

    const prompt = `My mood is "${currentMood}". Suggest a very short (one-sentence) motivation or tip for doing "${activity.title}" which ${activity.description}.`;
    const msg = await InvokeLLM(prompt);
    setAiTips((prev) => ({ ...prev, [activity.id]: msg }));
  };
          ...
          >
            <Card
              className={`relative overflow-hidden transition-all duration-300 cursor-pointer group
                ${matchesMood
                  ? "ring-2 ring-purple-400 bg-gradient-to-br from-white/90 to-purple-50/90"
                  : "bg-white/70"
                }
            ...
            >
              ...
              <CardHeader className="pb-3">
                <div
                  className={`w-12 h-12 rounded-xl bg-gradient-to-r ${activity.color} flex items-center justify-center mb-4 group-hover:scale-110 transition-transform`}
                >
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex gap-2">
                  <Button
                    ...
                    className={`w-full bg-gradient-to-r ${activity.color} hover:opacity-90 text-white font-medium py-3 rounded-xl transition-all duration-300 group-hover:shadow-lg`}
                  >
                    Get Started
                  </Button>
                  <Button
                    variant="outline"
                    onClick={() => getAiTip(activity)}
                    className="text-xs px-2 py-1"
                  >
                    {aiTips[activity.id] ? "✓" : "AI Tip"}
                  </Button>
                </div>
              </CardContent>
            </Card>
